...
- No generic troubleshooting — discriminating analysis only`,
    eval_augment: `TECHNICAL DOMAIN EVALUATION OVERLAY:
For REASONING_QUALITY: Are hypotheses ranked with explicit mechanisms? Is causal vs correlational reasoning distinguished?
For EVIDENCE_MAPPING: Is each claim tied to a specific observed fact? Are counter-indicators acknowledged?
For ACTIONABILITY: Is the highest-value next diagnostic step identified? Are validation tests concrete and testable?
For RISK_AWARENESS: Are misdiagnosis risks enumerated? Would the reasoning hold if one key fact changed?`
  },
  legal: {
    label: "Legal",
    system_augment: `Apply legal reasoning discipline with typed output structures:

ARGUMENT ATOMS (IRAC): For each legal issue, produce:
- Issue: the specific legal question
- Rule: the governing provision with pinpoint (Act, s, sub-s) and rule text
- Application: mapping of facts to rule elements, with burden/standard identified
- Conclusion: what follows from the application
- Admissibility risk: low/moderate/high with grounds (e.g., s 138 Evidence Act, hearsay, opinion)
- Supporting citations: source, jurisdiction, provision, pinpoint, quote range where available

LEGISLATIVE MAPPING: For any statutory framework, map the dependency chain:
Act → Regulations → Rules → Instruments → Forms → Practice Notes
Identify node_type (Act, Regulation, Rule, Instrument, Form, Case) and relation (implements, prescribes, authorises, constrains, interprets, overrides)

SALIENCE RANKING: For each issue/fact/defect, provide:
- Score (0-1 indicating relative importance)
- Explanation of why it matters
- Drivers (factors increasing salience)
- Counterfactors (factors reducing salience)

COMPLIANCE ANALYSIS: Where procedural requirements apply, classify non-compliance:
- Framework (CPA 2009, Evidence Act 2008, RSA 1986, Charter 2006, MCCR 2019, Sentencing Act 1991)
- Severity: technical (curable), material (affects rights), fundamental (power-defeating)
- Remedy: what the affected party can do

EXTRACTION: Identify parties, charges (provision + elements + max penalty), issues in fact, asserted defences, certificates with suspected defects

Citation standard: AGLC4 with pinpoints. Distinguish statute/regulation/case law/policy/assumption.
Prefer Victorian/Australian sources when matter indicates VIC. Jurisdiction-agnostic when unspecified.`,
    eval_augment: `LEGAL DOMAIN EVALUATION OVERLAY:
Score these additional legal-specific criteria within the 8 dimensions:

For INSTRUCTION_FIDELITY: Did the response produce typed argument atoms (IRAC structure) rather than unstructured prose?
For EPISTEMIC_INTEGRITY: Are legal propositions mapped to governing sources of law? Is statute distinguished from regulation from case law from policy from assumption?
For REASONING_QUALITY: Is the legislative dependency chain mapped (Act→Reg→Rule→Instrument)? Are competing interpretations considered?
For EVIDENCE_MAPPING: Are citations pinpointed (Act, section, subsection, paragraph)? Are admissibility risks flagged with specific grounds?
For TECHNICAL_PRECISION: Are provisions cited with correct pinpoint format? Is jurisdiction specified?
For ACTIONABILITY: Are remedies mapped to causes of action? Are procedural mechanics explained (how to raise, filing requirements, time limits)?
For RISK_AWARENESS: Is non-compliance severity classified (technical/material/fundamental)? Are consequences of gaps identified?

LEGAL AUTO-FAIL CONDITIONS (in addition to standard auto-fails):
- Citing a provision that does not exist → unusable (hallucinated authority)
- Stating a legal rule without any source of law → needs_revision
- Confusing jurisdiction (applying NSW law to a VIC matter without flagging) → needs_revision
- Missing a fundamental precondition failure when facts disclose one → high severity failure

SALIENCE-WEIGHTED CONTRADICTIONS: When the two responses disagree on a legal point, classify the contradiction severity:
- fundamental: affects the viability of a cause of action or defence
- material: affects the strength but not viability of a position
- technical: affects form but not substance

Return contradictions sorted by severity (fundamental first).`
  },
  reasoning_heavy: {
    label: "Reasoning",
...
- Provide confidence scores with justification`,
    eval_augment: `REASONING DOMAIN EVALUATION OVERLAY:
For EPISTEMIC_INTEGRITY: Is every substantive claim typed (observation/inference/speculation/conclusion/recommendation)? Are implicit assumptions surfaced?
For REASONING_QUALITY: Are competing explanations genuinely compared (not just listed)? Are logical chains explicit and traceable?
For EVIDENCE_MAPPING: Are "floating conclusions" (claims with no evidential anchor) present? Flag them.
For RISK_AWARENESS: Is counterfactual robustness addressed — would the conclusion change if one premise were wrong?`
  },
  mixed: {
    label: "Auto-detect",
...
- Flag risks of wrong conclusions`,
    eval_augment: `MIXED/GENERAL DOMAIN EVALUATION OVERLAY:
Apply the strictest applicable standard from each domain. If the query touches law, apply legal overlay criteria. If it involves systems/debugging, apply technical overlay. If it requires multi-step reasoning, apply reasoning overlay. Contradictions should be severity-classified where possible.`
  }
};
...
function buildEvaluatorPrompt(domain) {
  const p = DOMAIN_PROFILES[domain] || DOMAIN_PROFILES.mixed;
  return `You are an adversarial evaluation engine implementing the LLM Response Evaluation Matrix v1.0.
...
Respond ONLY in valid JSON (no fences):
{
  ...
  "response_b": { "...same..." },
  "contradictions": [{"point":"...", "a_says":"...", "b_says":"...", "resolution":"...", "severity":"fundamental|material|technical"}],
  "gaps_both_missed": ["..."],
...
function buildSynthesisPrompt(domain) {
  const p = DOMAIN_PROFILES[domain] || DOMAIN_PROFILES.mixed;
  const legalOverlay = domain === "legal" ? `

LEGAL OUTPUT STRUCTURE: When synthesising legal analysis, structure the output as:
1. Issues identified (with salience scores where determinable)
2. For each issue: Argument Atom (Issue → Rule with pinpoint → Application to facts → Conclusion)
3. Admissibility flags (risk level + specific grounds)
4. Legislative dependency map where relevant (Act → Reg → Rule → Instrument chain)
5. Compliance issues classified by severity (technical/material/fundamental)
6. Actionable next steps with procedural mechanics (what to file, when, consequences of not doing so)
7. Dissent Notes (where models disagreed, with contradiction severity: fundamental/material/technical)

Citation standard: AGLC4 with pinpoints. Every legal proposition must be anchored to a source of law.` : "";
...
2. Resolve contradictions per evaluation analysis — prioritise by severity (fundamental > material > technical)
...
async function callClaude(msgs, sys, max = 4096, apiKey = null) {
...
// ═══════════════════════════════════════════════════════════════════
// PERSISTENT STORAGE — keys survive across sessions via window.storage
// ═══════════════════════════════════════════════════════════════════

const STORAGE_KEY = "dual-ai-engine:api-keys";
...
async function saveKeys(openai, anthropic) {
  try {
    await window.storage.set(STORAGE_KEY, JSON.stringify({ openai, anthropic, saved_at: new Date().toISOString() }));
    return true;
  } catch (e) { console.error("Storage save failed:", e); return false; }
}

async function clearKeys() {
  try { await window.storage.delete(STORAGE_KEY); } catch { /* noop */ }
}
...
export default function DualAIEngine() {
  ...
  // Auto-load keys from persistent storage on mount
  useEffect(() => {
    (async () => {
      const stored = await loadKeys();
      if (stored?.openai) {
        setOpenaiKey(stored.openai);
        if (stored.anthropic) setAnthropicKey(stored.anthropic);
        setKeySet(true);
        setKeySource("stored");
      }
  ...
  // Save keys to persistent storage when manually entered
  const handleSaveKeys = useCallback(async () => {
    if (!openaiKey.trim()) return;
    const saved = await saveKeys(openaiKey.trim(), anthropicKey.trim() || "");
    setKeySet(true);
    setKeySource(saved ? "stored" : "manual");
  }, [openaiKey, anthropicKey]);

  // Clear stored keys and reset
  ...
  const run = useCallback(async () => {
    ...
    const t0 = Date.now();
    // Use anthropic key if provided, otherwise null (falls back to environment auth)
    const aKey = anthropicKey.trim() || null;
    try {
      // Classify
      let dom = domain;
      if (domain === "mixed") {
        const cl = await callClaude([{ role: "user", content: `Classify into exactly one: technical, legal, reasoning_heavy, mixed. One word only.\n\n${query}` }], "Task classifier. One word.", 20, aKey);
      ...
      const dt = Date.now();
      const [cR, gR] = await Promise.all([callClaude(um, sp, 4096, aKey), callOpenAI(openaiKey, um, sp)]);
      ...
      const eP = `Domain: ${dom}\n\nQuery:\n${query}\n\n--- Response A ---\n${aR}\n\n--- Response B ---\n${bR}`;
      const eRaw = await callClaude([{ role: "user", content: eP }], buildEvaluatorPrompt(dom), 6000, aKey);
      let ev;
      ...
      if (ev.follow_up_needed?.needed && ev.follow_up_needed?.query) {
        ...
        const [fc, fg] = await Promise.all([callClaude([{ role: "user", content: fq }], sp, 2048, aKey), callOpenAI(openaiKey, [{ role: "user", content: fq }], sp, 2048)]);
      ...
      if (fu) si += `\n\n--- Follow-up ("${fu.query}") ---\n1: ${fu.claude}\n2: ${fu.gpt}`;
      const syn = await callClaude([{ role: "user", content: si }], buildSynthesisPrompt(dom), 4096, aKey);
  ...
  }, [query, openaiKey, anthropicKey, domain]);

  // ── Loading ──
  if (keysLoading) return (
    <div style={S.root}><div style={S.keyScreen}>
      <div style={S.mark}>◈</div>
      <p style={{color:"#888",fontSize:14}}>Loading stored credentials...</p>
    </div></div>
  );
  ...
  if (!keySet) return (
    <div style={S.root}><div style={S.keyScreen}>
      ...
      <div style={S.keyCard}>
        <label style={S.lbl}>OpenAI API Key <span style={{color:"#f87171"}}>*</span></label>
        <input type="password" value={openaiKey} onChange={e => setOpenaiKey(e.target.value)} placeholder="sk-..." style={S.inp} />

        <div style={{height:8}}/>

        <label style={S.lbl}>Anthropic API Key <span style={{color:"#888",fontWeight:400,textTransform:"none"}}>(optional in claude.ai)</span></label>
        <input type="password" value={anthropicKey} onChange={e => setAnthropicKey(e.target.value)} placeholder="sk-ant-..." style={S.inp}
          onKeyDown={e => e.key === "Enter" && openaiKey.trim() && handleSaveKeys()} />

        <p style={S.hint2}>Both keys saved to persistent storage. Auto-load in future sessions. Only transmitted to their respective APIs.</p>
        <button onClick={handleSaveKeys} disabled={!openaiKey.trim()}
          style={{ ...S.pri, ...(openaiKey.trim() ? {} : S.dis) }}>Save &amp; Initialise</button>
      </div>
    </div></div>
  );
  ...
  return (
    <div style={S.root}>
      ...
      <div style={S.hdr}>
        ...
        <div style={S.hdrR}>
          <span style={S.mbadge}>Sonnet 4</span><span style={S.mbadge}>GPT-4o</span>
          {keySource === "stored" && <span style={{fontSize:10,color:"#4ade80",padding:"3px 7px",borderRadius:3,background:"rgba(74,222,128,0.08)",border:"1px solid rgba(74,222,128,0.2)"}}>
            {anthropicKey ? "Both keys" : "OpenAI key"} stored
          </span>}
          <button onClick={handleClearKeys} style={S.ghost}>Clear Keys</button>
        </div>
      </div>
      {/* Tabs */}
      <div style={S.tabs}>
        ...
        {showBB && sr.dispatch && <div style={S.bb}>
          ...
          {ev.contradictions?.length > 0 && <div style={S.bbSec}>
            ...
            {ev.contradictions.map((c,i) => {
              const sevColor = c.severity === "fundamental" ? "#ef4444" : c.severity === "material" ? "#f59e0b" : "#888";
              return <div key={i} style={S.cCard}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                  <span style={{fontWeight:600}}>{c.point}</span>
                  {c.severity && <span style={{fontSize:10,fontWeight:700,textTransform:"uppercase",color:sevColor,padding:"2px 6px",borderRadius:3,border:`1px solid ${sevColor}30`}}>{c.severity}</span>}
                </div>
